...
크기 한계에 걸릴 수 있다. 그래서 "100%" 버튼은 화면 중앙 1200x1200px 영역만
보여준다(핀치 줌/팬 가능). 탭해서 보고 싶은 지점을 고르는 기능은 아직 없음 —
항상 중앙 기준.

초기 구현은 이 패치를 보여주기 위해 **전체 이미지**를 보정(화이트밸런스~필름
시뮬레이션)한 뒤 마지막에 잘라내는 방식이어서, 1억 화소 이미지에서 중간 버퍼가
GB 단위로 쌓여 저사양 기기에서 OOM으로 앱이 죽는 문제가 있었다. 지금은
`raw_process.cpp`가 요청한 패치(+회전)에 대응하는 원본 좌표계 사각형을 먼저
역산해서, 그 작은 영역(수 MB)만 보정한다 — 원본 전체 디코드 버퍼(LibRaw가 항상
센서 전체를 디모자이킹하므로 이건 피할 수 없음)를 빼면 나머지 메모리 사용량은
패치 크기에 비례한다.
